...
use App\Models\SalesOrdersProduct;
...
new class extends Component {
    ...
    public array $order_emails = [];
    public array $selected_order_emails = [];
    public string $new_order_email = '';
    ...
    public array $selected_lines = [];
    public array $selected_lines_units = [];
    ...
    public function mount(?int $id = null): void
    {
        ...
        if ($id) {
            $this->order = SalesOrder::emtGet(
                model_id: $id,
                with: ['products.product_variant.product.tax_type']
            );
            $this->order_emails = [$this->order->thirdparty->email];
            $this->selected_order_emails = [$this->order->thirdparty->email];
            $this->isEditing = true;
            $this->loadOrderData();
        } else {
    ...
    /**
     * Load order data into properties
     */
    private function loadOrderData(): void
    {
        ...
        $this->internal_note = $this->order->internal_note;
        $this->vat = $this->order->vat;
        $this->legal_form = $this->order->legal_form;
        $this->address = $this->order->address;
        $this->zip = $this->order->zip;
        $this->town = $this->order->town;
        ...
        $this->tax_retention = $this->order->tax_retention ?? 0.0;
    }
    ...
    public function with(): array
    {
        return [
            'pageTitle' => $this->isEditing
                ? __('sales.edit_order')
                : __('sales.create_order'),
            'breadcrumbs' => [
                ['label' => trans_choice('sales.sales', 2), 'url' => null],
                ['label' => trans_choice('sales.orders', 2), 'url' => route('sales.orders.index')],
                ['label' => $this->isEditing ? $this->order->number : __('general.new'), 'url' => null],
            ],
            'products_summary' => $this->order ? $this->order->emtGetProductsSummary() : [],
        ];
    }
    ...
    public function downloadPDF()
    {
        ...
        return response()->streamDownload(
            fn () => print($data),
            'Pedido-'.$this->order->number.'.pdf',
            ['Content-Type' => 'application/pdf']
        );
    }
    public function sendEmail()
    {
        if (length($this->selected_order_emails) == 0) {
    ...
    public function createOrderEmail()
    {
        $validated = $this->validate([
            'new_order_email' => 'required|email:rfc,dns',
        ]);
        $this->order_emails[] = $validated['new_order_email'];
    }
    ...
    public function loadLines(): void
    {
        ...
        $this->lines = $this->order->products->toArray();
    }
    ...
    public function editLine(int $lineId): void
    {
        ...
        $product_line = $this->order->products->where('id', $lineId)->first();
    ...
    {
        if (!$this->order || !$this->order->id) {
            Flux::toast(variant: 'danger', text: __('sales.save_order_first'));
            return;
        }
        ...
        $es_type = ($this->order->thirdparty->equivalence_surcharge ?? 0) ? $tax_type->pes : 0;
        ...
        if ($this->editingLineId) {
            ...
            $line = SalesOrdersProduct::find($this->editingLineId);
            $line->update($lineData);
            Flux::toast(variant: 'success', text: __('sales.line_updated'));
        } else {
            // Create new line
            SalesOrdersProduct::create($lineData);
    ...
    {
        if ($this->deleteId) {
            $line = SalesOrdersProduct::find($this->deleteId);
    ...
    public function initCreateNote()
    {
        $this->selected_lines = [];
        $this->customer_date = today()->format('Y-m-d');
        foreach ($this->order->products as $line) {
            $this->selected_lines[] = $line->id;
            $this->selected_lines_units[$line->id] = $line->units;
        }
        $this->openCreateNote = true;
    }
    public function createNote()
    {
        $selected_lines_units = [];
        foreach ($this->selected_lines as $line_id) {
            if (isset($this->selected_lines_units[$line_id]) && $this->selected_lines_units[$line_id] != 0) {
                $selected_lines_units[$line_id] = $this->selected_lines_units[$line_id];
            }
        }
        $this->order->createNote($selected_lines_units, $this->customer_date);
        Flux::toast(variant: 'success', text: __('sales.note_created'));
        $this->selected_lines = [];
        $this->selected_lines_units = [];
    }
    public function updateLineUnits($lineId, $units)
    {
        $this->selected_lines_units[$lineId] = $units;
    }
    ...
    public function save(): void
    {
        ...
        if ($this->isEditing) {
            ...
            $this->loadOrderData();
            Flux::toast(variant: 'success', text: __('sales.order_updated'));
        } else {
            $thirdparty = Thirdparty::emtGet($this->thirdparty_id);
            $validated['tax_retention'] = $thirdparty->tax_retention;
            // Create new order
            $this->order = SalesOrder::create($validated);
            Flux::toast(variant: 'success', text: __('sales.order_created'));
            $this->redirect(route('sales.orders.edit', ['id' => $this->order->id]));
        }
    }

    /**
     * Cancel and return to orders list
     */
    ...
    <x-document.layout :title="$pageTitle" :breadcrumbs="$breadcrumbs">
                        ...
                        <flux:tab.panel name="lines-tab" class="pt-1!">
                            <div class="space-y-4">
                                <div class="flex justify-end gap-2">
                                    ...
                                    <flux:button size="sm" variant="primary" color="blue" wire:click="initCreateNote" icon="file-plus" class="cursor-pointer">
                                        {{ __('sales.create_note') }}
                                    </flux:button>
                                </div>
                                ...
                                @if(count($lines) > 0)
                                    <flux:table>
                                        <flux:table.columns sticky>
                                            ...
                                            @if($this->order->thirdparty->equivalence_surcharge ?? 0)
        ...
        <x-slot:modals>
            ...
            <flux:modal class="md:w-96" name="send-email" wire:model.self="openSendEmail">
                <div class="space-y-6">
                    <div>
                        <flux:heading size="lg">{{ __('sales.send_order') }}</flux:heading>
                    </div>
                    <div>
                        <flux:pillbox size="sm" wire:model="selected_order_emails" variant="combobox" multiple label="Email">
                            <x-slot name="input">
                                <flux:pillbox.input size="sm" wire:model="new_order_email" placeholder="Email" />
                            </x-slot>
                            @foreach ($this->order_emails as $order_email)
                                <flux:pillbox.option :value="$order_email">{{ $order_email }}</flux:pillbox.option>
                            @endforeach
                            <flux:pillbox.option.create wire:click="createOrderEmail" min-length="3">
                                {{ __('general.add') }} "<span wire:text="new_order_email"></span>"
                    ...
                    <div class="flex gap-2">
                        ...
                        <flux:modal class="w-[95%]! md:w-auto! md:max-w-[50vw]!" name="create-note" wire:model.self="openCreateNote">
                <div class="space-y-6">
                    <div>
                        <flux:heading size="lg">{{ __('sales.create_note') }}</flux:heading>
                    </div>
                    <div class="space-y-4 grid grid-cols-1 lg:grid-cols-6 gap-4">
                        <div class="col-span-1 lg:col-span-2">
                            <flux:date-picker label="{{ __('sales.customer_date') }}" size="sm"
                                wire:model="customer_date" locale="es-ES" start-day="1" error="customer_date" />
                        </div>

                        <div class="col-span-1 lg:col-span-6 overflow-x-auto">
                            <flux:checkbox.group>
                                <flux:table container:class="max-h-[calc(100vh-22rem)]" class="table-fixed w-full">
                                    <flux:table.columns sticky class="bg-blue-50 dark:bg-zinc-900 mx-2">
                                        <flux:table.column align="center" class="w-16">
                                            <flux:checkbox.all class="pl-1 md:pl-0" />
                                        </flux:table.column>
                                        <flux:table.column class="w-24">{{ __('sales.units') }}</flux:table.column>
                                        <flux:table.column>{{ __('sales.description') }}</flux:table.column>
                                    </flux:table.columns>
                                    <flux:table.rows>
                                        @foreach ($this->order->products as $line)
                                            <flux:table.row :key="$line->id">
                                                <flux:table.cell class="w-16">
                                                    <flux:checkbox wire:model="selected_lines" value="{{ $line->id }}" />
                                                </flux:table.cell>
                                                <flux:table.cell class="w-24">
                                                    <flux:input size="sm"
                                                        wire:change="updateLineUnits({{ $line->id }}, $event.target.value)"
                                                        value="{{ $line->units }}"
                                                        mask:dynamic="$money($input, ',')" />
                                                </flux:table.cell>
                                                <flux:table.cell class="truncate">{{ $line->description }}</flux:table.cell>
                                            </flux:table.row>
                                        @endforeach
                                    </flux:table.rows>
                                </flux:table>
                    ...
                    <div class="flex gap-2">
                        <flux:spacer />
                        <flux:modal.close>
                            <flux:button size="sm" variant="primary" color="blue" wire:click="createNote" icon="file-plus">
                                {{ __('sales.create_note') }}
                            </flux:button>
                        </flux:modal.close>
                    </div>
