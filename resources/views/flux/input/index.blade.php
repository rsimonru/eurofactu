...
@php
// Detect if mask:dynamic is for money formatting
$useAutonumeric = $maskDynamic && str_contains($maskDynamic, '$money');
$autonumericId = $useAutonumeric ? 'autonumeric-' . uniqid() : null;
@endphp
...
<?php elseif ($as !== 'button'): ?>
    @php
        ...
        if ($wireModelAttr) {
            // Extract property name from wire:model, wire:model.live, wire:model.defer, etc.
            $wireModelName = preg_replace('/^wire:model(\.\w+)?\.?/', '', $wireModelAttr);
            $wireModelName = trim($wireModelName, '.');
        } else {
            $wireModelName = null;
        }
    @endphp
    <flux:with-field :$attributes :$name>
        <div
            {{ $attributes->only('class')->class('w-full relative block group/input') }}
            data-flux-input
            @if ($useAutonumeric && $wireModelName)
            x-data="{
                    ...
                    if (input && window.AutoNumeric) {
                        ...
                        this.$watch('$wire.' + this.wireModelName, (value) => {
                            if (value !== null && value !== undefined && value !== '') {
                                ...
                                const currentValue = this.autonumericInstance.getNumber();
                                if (!isNaN(numValue) && currentValue !== numValue) {
                                    this.autonumericInstance.set(numValue);
                                }
